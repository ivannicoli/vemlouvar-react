...
async function carregarLastMissa() {
  ...
  const promises = lastMissa.musicas.map(async momentoMusica => {
    ...
    musicaDoc = await firebase.firestore().collection('musicas').doc("" + momentoMusica.musica).get();
...
function generateJsGridHtml() {
  ...
  $("#jsGrid").jsGrid({
    ...
    rowClass: function (item, itemIndex) {
      return "client-" + itemIndex + " ui-icon-arrowthick-2-n-s";
    },
    ...
    onRefreshed: function () {
      ...
      $gridData.sortable({
        update: function (e, ui) {
          ...
          // arrays of items
          musicasGrid = $.map($gridData.find("tr"), function (row) {
